...
public class Paddle : MonoBehaviour {
    ...
    //스테이지 초기화 ( -1 재시작, 0 다음스테이지, 숫자 스테이지)
    ...
    void BlockGenerator()
    {
        ...
        for (int i = 0; i < currentStr.Length; i++)
        {
            ...
            else currentB = CharUnicodeInfo.GetDecimalDigitValue(A);
    ...
    {
        // 같은 볼끼리 충돌 무시
        Physics2D.IgnoreLayerCollision(2, 2); // 2와 2는 서로의 충돌을 무시한다
        ...
        switch (Col.name)
        {
            ...
            case "MagnetPaddle":
                ...
                {
                    if (Input.GetMouseButton(0) || (Input.touchCount == 1 && Input.GetTouch(0).phase == TouchPhase.Moved))
                        ThisBallTr.position = new Vector2(transform.position.x + gapX, ThisBallTr.position.y); // 마그넷 핵심코드
    ...
    public void BallAddForce(Rigidbody2D ThisBallRg)
    {
        ...
        ThisBallRg.velocity = Vector2.zero; //순간적으로 힘을 0으로 만든 다음에 힘을 받아야 더 잘 움직이기 때문에 넣어준다
